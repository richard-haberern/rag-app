...
def test_chunker_1(fake_tokenizer):
    ...
    assert chunks == ["w01 w02 w03 w04 w05 w06 w07 w08 w09 w10 w11 w12 w13 w14 w15 w16 w17 w18 w19 w20",
...
def test_max_size(fake_tokenizer):
    with pytest.raises(ValueError):
        chunker = Chunker(fake_tokenizer, 0, 5)

def test_overlap(fake_tokenizer):
    with pytest.raises(ValueError):
        chunker = Chunker(fake_tokenizer, 20, -1)

def test_overlap_bigger_size(fake_tokenizer):
    with pytest.raises(ValueError):
        chunker = Chunker(fake_tokenizer, 20, 25)

def test_overlap_equal_size(fake_tokenizer):
    with pytest.raises(ValueError):
        chunker = Chunker(fake_tokenizer, 20, 20)
